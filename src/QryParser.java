...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        if(operatorNameLowerCase.indexOf('/') > 0) {
            String[] strings = operatorNameLowerCase.split("/");
            operatorNameLowerCase = strings[0];
            operatorDistance = Integer.valueOf(strings[1]);
        }//System.out.println(operatorNameLowerCase);
    ...
    private static Qry[] createTerms(String token) throws IOException {
        ...
        if ((field.compareTo("url") != 0) && (field.compareTo("keywords") != 0) && (field.compareTo("title") != 0)
                && (field.compareTo("body") != 0) && (field.compareTo("inlink") != 0) && !(isNumeric(field))) {
    ...
    private static Qry parseString(String queryString) throws IOException, IllegalArgumentException {
        ...
        while (queryString.length() > 0) {
            ...
            if(isNumeric(popTerm(queryString).getPopped())) {
                weight = Double.parseDouble(popTerm(queryString).getPopped());
                queryString = popTerm(queryString).getRemaining().trim();
            }
...
//System.out.println(queryString);
            ...
            //queryString
            //System.out.println(popTerm(queryString).getPopped());
            ...
            if(qargs != null) {
                for (int i = 0; i < qargs.length; i++) {
                    ...
                    // HW2
                    //System.out.println(weight);
                    //qargs[i].setWeight(weight);
                    queryTree.appendArg(qargs[i]);
                    queryTree.args.get(index).setWeight(weight);
                    index++;
                }
            }
        }
